...
export class DataSiswaService {
  ...
  async getAllDataSiswaBySiswaId(siswa_id: string[]) {
    try {
      const status = STATUS_SISWA.PENDAFTAR;
      return await this.dataSiswaRepository.findAllBySiswaId(siswa_id, status);
    } catch (error) {
      throw error;
    }
  }
