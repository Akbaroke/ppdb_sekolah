...
import { biayaDUSBDummy } from '../../../data/dummy';
...
export default function DetailBiayaDUSB() {
  const navigate = useNavigate();
  const rows = biayaDUSBDummy?.map((element) => (
    <Table.Tr key={element.id}>
      ...
      <Table.Td className="flex items-center justify-evenly gap-3">
        <ModalForm title="Edit Biaya DUSB" formType="biaya_dusb" data={element}>
          <ActionIcon variant="light" size="lg">
            <IconPencil size={18} />
          </ActionIcon>
        </ModalForm>
        <ModalConfirm
          title="Hapus Biaya DUSB"
          icon={<IconTrash />}
          type="danger"
          text={
            <>
              Apakah anda yakin ingin menghapus biaya DUSB ini "
    ...
    <div className="flex flex-col gap-5">
      <Card className="flex justify-between items-center flex-wrap gap-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <ModalForm title="Buat Biaya DUSB" formType="biaya_dusb">
            <ActionIcon variant="light" size="lg">
              <IconPlus size={18} />
            </ActionIcon>
          </ModalForm>
